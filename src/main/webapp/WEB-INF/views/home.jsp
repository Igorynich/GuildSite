...
<div class="mainform">
    ...
    <sf:form method="POST" commandName="zayavka" acceptCharset="UTF-8">
        ...
        <sf:errors path="*" element="div" cssClass="alert alert-danger"></sf:errors>

        <sf:label path="gstatic" cssErrorClass="alert alert-danger"><s:message code="form.gstatic"/></sf:label>
        <sf:radiobuttons path="gstatic.staticName" cssErrorClass="alert alert-danger" items="${staticlist}" itemLabel="staticName" itemValue="staticName"/><br/>

        <sf:label path="name" cssErrorClass="alert alert-danger"><s:message code="form.name"/></sf:label>
        <sf:input path="name" cssErrorClass="alert alert-danger"/> <br/>
                            ...
                            <c:forEach var="spec" items="${specList}">
                                <c:forEach var="wcls" items="${spec.wowClassLoc}">
                                    <c:if test="${wcl.wowClassLocalized eq wcls.wowClassLocalized}">
                                        <option value="${spec.shortName}" label="${wcls.wowSpecLocalized}"/>
    ...
    <sec:authorize access="isAnonymous()">
        <div class="right-side">
            ...
            <a class="btn" href="${login}"><s:message code="form.login"/></a>
        </div>
    </sec:authorize>
    <sec:authorize access="isAuthenticated()">
        <div class="right-side">
            ...
            <sf:form action="/logout" method="post">
                <input type="submit" class="btn" value="<s:message code="form.logout"/>"/>
            </sf:form>
        </div>
    </sec:authorize>
</div>
